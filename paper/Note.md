...
2. 对一个神经网络模型进行的测试条件。这个模型使用修改后的图像集进行训练，并在三种条件下进行测试。
...
3. 他们使用了一个16层的VGG网络和一个101层的ResNet网络，这两个网络都是用PyTorch、Keras和TensorFlow的torchvision包提供的。
